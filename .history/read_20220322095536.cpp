...
pair<set<int>,vector<double>> greedy(Graph g, int k, int mc, double epsilon){
   ifstream outfile ("log/log.txt");
   auto timenow = chrono::system_clock::to_time_t(chrono::system_clock::now());
   cout << ctime(&timenow) << endl;
   cout<<"graph with "<<g.numVert<<" nodes, "<< g.numEdge<<" edges."<<endl;
   cout<<"Algorithm: initial greedy"<<endl;
   cout<<"Budget: "<< k<<endl;
   cout<< "MC rounds = "<<mc<<"| Possibility of influence = normal | Epsilon = "<< epsilon <<endl;
   ...
   std::cout << "Time difference = " << std::chrono::duration_cast<std::schrono::microseconds>(end - begin).count() << "[µs]" << std::endl;
