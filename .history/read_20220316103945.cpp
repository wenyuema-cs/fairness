...
void RR(Graph g, int rrt_rr, int rrt_set, int SAMPLE_SIZE, int threshold, int budget){
   set<int> S, G, rS;
   vector<int> rG;
   vector<double> maxinf;
   for(int i; i<g.numVert;i++){
   ...
   while(!rG.empty()){
      ...
      vector<int> rr_list;
      while((target_list.size()< SAMPLE_SIZE) && (target_list.size() != rG.size())){
...
int main()
{
   ...
   ifstream infile ("networksdata.txt");
   int u,v;
   double p;
   if (infile.is_open())
   {
