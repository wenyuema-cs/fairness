...
void rrSelect(Graph g, int SAMPLE_SIZE, int SAMPLE_ROUND){
   set<int> seed;
   int counter [g.numVert];
   int V [g.numVert];
   for (int i =0;i<g.numVert;i++){
   ...
   cout<< "couter itself, it length as"<<counter.size()<<endl;
   for(const auto&_:counter){
      cout<<_<<" ";
   }
   cout<<endl;   

   sort( V,V+g.numVert, [&](int i,int j){return counter[i]<counter[j];} );
...
int main()
{
   ...
   */
   ...
   /**/
   // test of RR
   rrSelect(g ,SAMPLE_SIZE, SAMPLE_ROUND);
