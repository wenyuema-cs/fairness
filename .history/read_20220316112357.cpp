...
pair<int,int> icExp(Graph g, queue<int> s,int mc,double epsilon){
...
pair<set<int>,vector<double>> greedy(Graph g, int k, int mc, double epsilon){
   ...
   for(int _ =0; _<k;_++){
      ...
      for (j = G.begin(); j != G.end(); j++) {
         ...
         pair<int,int> res = icExp(g, s, mc, epsilon);
         int exp_j=res.first;
...
void RR(Graph g, int rpt_rr, int rpt_set, int SAMPLE_SIZE, int threshold, int budget){
   set<int> S, G, rS;
   vector<int> sG;
   ...
   while(!Q.empty()){
      int u = Q.front();
      Q.pop();
      for(int w = 0; w < g.nxt[u].size; w++){
         if((g.nxt_prob[u][w] <randam()) && (g.act[g.nxt[u][w]]==0)){
            g.act[g.nxt[u][w]] = 1;
            Q.push(g.nxt[u][w]);
            sG.push_back(g.nxt[u][w]);
...
int main()
{
   ...
   */
   ...
   /* */
   // test of greedy
   pair<set<int>,vector<double>> res = greedy(g, 2,100,0.2);
