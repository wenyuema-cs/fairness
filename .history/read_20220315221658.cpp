...
pair<int,int> icExpq(Graph g, queue<int> s,int mc,double epsilon){
   ...
   //cout<<"--------------------------------------"<<endl;
   int min_expect = min_element(counter, counter+g.numVert);
   int num = 0;
   for(const auto& count : counter){
      cout<<count<<" ";
...
// void greedy(Graph g, int k, int mc, double epsilon){
//    queue<int> S;
...
int main()
{
   ...
   //Model ic;
   pair<int,int> res=icExp(g,selectq,100,0);
