...
void RR(Graph g, int SAMPLE_SIZE, int SAMPLE_ROUND){
   ...
   for(int i = 0; i < 2; i++){
      cout<<"in for of i"<<endl;
      double rand = g.numVert * (float)randam();
      cout<<"seed "<<rand<<" as the initial seed we selected.";      
      int seed  = (int)rand;
      cout<<"seed "<<seed<<" as the initial seed we selected.";
      Q.push(seed);

      int j=0;
      while((!Q.empty()) && S[i].size() < SAMPLE_SIZE){
