      ...
      for(const auto& candi : s){
         ...
         for (int j =0; j < g.nxt[candi].size();j++){
            if(g.nxt_prob[candi][j] < randam()){
               ...
               cout<<"is larger than"<<"node "<<g.nxt[candi][j]<<" as probability"<< g.nxt_prob[candi][j]<<endl;

               

            }
            cout<<"node "<<g.nxt[candi][j]<<" could be activate by edge with probability"<< g.nxt_prob[candi][j]<<endl;
