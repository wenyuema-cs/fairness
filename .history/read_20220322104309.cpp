...
pair<set<int>,vector<double>> greedy(Graph g, int k, int mc, double epsilon){
   ...
   if(outfile.is_open()){
      outfile << ctime(&timenow) << endl;
      cout<<"Graph with "<<g.numVert<<" nodes, "<< g.numEdge<<" edges."<<endl;
      cout<<"Algorithm: initial greedy | Budget: "<< k<<endl;
      cout<< "MC rounds = "<<mc<<"| Possibility of influence = normal | Epsilon = "<< epsilon ;
   }
   ...
   cout<<"Sorted selecting nodes: "<<endl;
