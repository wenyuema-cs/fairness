...
void icExp(Graph g, vector<int> s,int mc,double epsilon){
   for(int i = 0; i<mc; i++){
      for(const auto& candi : s){
         ...
         for (int j =0; j < g.pre[candi].size();j++){
            if(g.nxt_prob[candi][j] < randam()){
               cout
               g.act[g.pre[candi][j]]=true;
               if (g.act[g.pre[candi][j]]=true){
                  cout<<g.pre[candi][j]<<"node as activated node"<<endl;

               }
               cout<<"node "<<j<<" has been activate by edge with probability"<< g.nxt_prob[candi][j]<<endl;
