...
pair<set<int>,vector<double>> greedy(Graph g, int k, int mc, double epsilon){
   ...
   auto start = system_clock::now();
   cout<<"Algorithm runs in "<< double(duration.count())*microseconds::period::num/microseconds::period<<"seconds"<<endl;
   return make_pair(set,maxinf);
...
int main()
{
   ...
   */
   ...
   /* */
   // test of greedy
   pair<array<int>,vector<double>> res = greedy(g, BUDGET, MCROUNDS, EPSILON);
   cout<<"Selecting nodes: "<<endl;
   // for (set<int, greater<double> >::iterator i = res.first.begin();i != res.first.end();i++){
   //    cout<<*i<<" ";
   // }
   for (double i =0;i<res.first.size();i++){
      cout<<res.first[i]<<" ";
   }
   cout<<endl;
